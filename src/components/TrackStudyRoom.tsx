        ...
        <div className="absolute top-0 left-0 right-0 h-16 bg-gray-800/90 backdrop-blur-sm border-b border-gray-700 flex items-center justify-between px-6 z-10">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 bg-gradient-to-br from-[#00c2ff] to-blue-600 rounded-lg flex items-center justify-center">
              ...
              >
                ...
                <div className="p-4 border-b border-gray-700 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="bg-[#00c2ff]/20 text-[#00c2ff] px-2 py-0.5 rounded-full text-xs font-medium">
                      {participants.length}
                    </span>
                  </div>
                  ...
                  {participants.map((participant) => (
                    <div
                      ...
                      className={`flex items-center justify-between p-3 rounded-xl mb-1 ${
                        participant.isSpeaking ? 'bg-[#00c2ff]/10 border border-[#00c2ff]/30' : 'hover:bg-gray-700/50'
                      } transition-all`}
                    >
                      <div className="flex items-center gap-3">
                        <div className={`relative w-10 h-10 rounded-full bg-gradient-to-br from-gray-600 to-gray-700 flex items-center justify-center ${
                          participant.isSpeaking ? 'ring-2 ring-[#00c2ff]' : ''
                        }`}>
                          <span className="text-white font-medium text-sm">
                            {participant.name.split(' ').map(n => n[0]).join('')}
                          </span>
                          {participant.isSpeaking && (
                            <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-[#00c2ff] rounded-full flex items-center justify-center">
              ...
              <div className="flex-1 bg-white rounded-2xl overflow-hidden relative">
                ...
                <div className="absolute top-4 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-gray-900/90 backdrop-blur-sm px-4 py-2 rounded-xl z-10">
                  <button
                    ...
                    className={`p-2 rounded-lg transition-colors ${whiteboardTool === 'pen' ? 'bg-[#00c2ff] text-white' : 'text-gray-400 hover:bg-gray-700'}`}
                  >
                    <PenTool size={18} />
                  </button>
                  <button
                    onClick={() => setWhiteboardTool('eraser')}
                    className={`p-2 rounded-lg transition-colors ${whiteboardTool === 'eraser' ? 'bg-[#00c2ff] text-white' : 'text-gray-400 hover:bg-gray-700'}`}
                  >
                    <Eraser size={18} />
                  </button>
                  <button
                    onClick={() => setWhiteboardTool('square')}
                    className={`p-2 rounded-lg transition-colors ${whiteboardTool === 'square' ? 'bg-[#00c2ff] text-white' : 'text-gray-400 hover:bg-gray-700'}`}
                  >
                    <Square size={18} />
                  </button>
                  <button
                    onClick={() => setWhiteboardTool('circle')}
                    className={`p-2 rounded-lg transition-colors ${whiteboardTool === 'circle' ? 'bg-[#00c2ff] text-white' : 'text-gray-400 hover:bg-gray-700'}`}
                  >
                    <Circle size={18} />
                  </button>
                  <button
                    onClick={() => setWhiteboardTool('text')}
                    className={`p-2 rounded-lg transition-colors ${whiteboardTool === 'text' ? 'bg-[#00c2ff] text-white' : 'text-gray-400 hover:bg-gray-700'}`}
                  >
                    <Type size={18} />
                  </button>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                  {chatMessages.map((msg) => (
                    <div key={msg.id} className="flex items-start gap-3">
                      <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#00c2ff] to-blue-600 flex items-center justify-center flex-shrink-0">
                ...
                <div className="p-4 border-t border-gray-700">
                  <div className="flex items-center gap-2 bg-gray-700/50 rounded-xl p-2">
                    ...
                    <button
                      ...
                      className="p-2 bg-[#00c2ff] hover:bg-[#00a8e0] rounded-lg transition-colors"
                    >
        ...
        <div className="absolute bottom-0 left-0 right-0 h-20 bg-gray-800/90 backdrop-blur-sm border-t border-gray-700 flex items-center justify-center gap-2 px-6">
          ...
          <div className="flex flex-col items-center">
            <button
              ...
              className={`w-12 h-12 rounded-full flex items-center justify-center transition-colors ${
                showWhiteboard ? 'bg-[#00c2ff] hover:bg-[#00a8e0]' : 'bg-gray-700 hover:bg-gray-600'
              }`}
            >
