...
function ResultsPhase({ programs, tracks, totalChapters, totalTracks, totalPrice, totalOriginalPrice, discount }: ResultsPhaseProps) {
  ...
  return (
    ...
    >
      {/* ========== LIGNE 1: Tout intégré (FULL WIDTH) ========== */}
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.2 }}
        className="bg-[#1a1f24] rounded-2xl border border-gray-700/50 p-8"
      >
        ...
        <p className="text-[16px] !text-white mb-8 leading-relaxed" style={{ fontFamily: 'var(--font-dm-sans)' }}>
          En débloquant un programme, tu peux générer <span className="text-[#00c2ff]">autant de parcours que nécessaire</span> pour 
          tous tes besoins, à vie.
        </p>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
          <button 
            ...
            className="px-8 py-4 bg-[#00c2ff] hover:bg-[#3ab5dc] text-white font-semibold rounded-full transition-all flex items-center justify-center gap-2"
          >
            Teste ton parcours – 10h offertes
            <ArrowRight size={18} />
          </button>
          
          <button className="px-8 py-4 bg-transparent border border-gray-600 hover:border-gray-500 text-white/80 font-medium rounded-full transition-all flex items-center justify-center gap-2">
